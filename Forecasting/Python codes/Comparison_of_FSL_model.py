...
df4 = pd.read_csv(r'C:\Users\HP\Documents\4th year\7th sem\EE405\DATA\Colombo_Dengue.csv')
print(df4)
df4.info()
#df['Month'] = pd.to_Month(df['Month'], infer_Month_format=True)
#df.set_index('Month')[['Rainfall', 'Temp', 'pop,000', 'Cases']].plot(subplots=True)
#plt.show()
df4_input = df4[['Rainfall', 'Temp','Humidity', 'Cases']]
print(df4_input)
print(df4_input.describe())

scaler = MinMaxScaler()
data_scaled = scaler.fit_transform(df4_input)
print(data_scaled)
...
x=X_test[:, :3][12:]
print(x)
df4_pred=pd.concat([pd.DataFrame(x), pd.DataFrame(predictions)], axis=1)
rev_trans=scaler.inverse_transform(df4_pred)
print(rev_trans)
df4_final=df4_input[predictions.shape[0]*-1:]
print(df4_final.count())
df4_final['predi_cases']=rev_trans[:, 3]
print(df4_final)
...
df5 = pd.read_csv(r'C:\Users\HP\Documents\4th year\7th sem\EE405\DATA\Colombo_Dengue.csv')
print(df5)
df5.info()
#df['Month'] = pd.to_Month(df['Month'], infer_Month_format=True)
#df.set_index('Month')[['Rainfall', 'Temp', 'pop,000', 'Cases']].plot(subplots=True)
#plt.show()
df5_input = df5[['Rainfall', 'Temp','Humidity', 'Cases']]
print(df5_input)
print(df5_input.describe())

scaler = MinMaxScaler()
data_scaled = scaler.fit_transform(df5_input)
print(data_scaled)
...
x=X_test[:, :3][12:]
print(x)
df5_pred=pd.concat([pd.DataFrame(x), pd.DataFrame(predictions)], axis=1)
rev_trans=scaler.inverse_transform(df5_pred)
print(rev_trans)
df5_final=df5_input[predictions.shape[0]*-1:]
print(df5_final.count())
df5_final['predi_cases']=rev_trans[:, 3]
print(df5_final)
...
ax.plot(day_pred, df5_final['predi_cases'], 'g--', marker='o', label='LSTM-FSL-1')
ax.plot(day_pred, df_final['predi_cases'], 'm--', marker='o', label='LSTM-FSL-2')
ax.plot(day_pred, df4_final['predi_cases'], 'red', marker='o',label='LSTM-FSL-3 ')
